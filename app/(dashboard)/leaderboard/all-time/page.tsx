...
export default function AllTimeLeaderboardPage() {
  ...
  // Fetch a window of draw history to compute biggest single win
  ...
  const totals = (() => {
    try {
      const totalWinnings = entries.reduce((acc: bigint, e: any) => acc + BigInt(e.totalWinnings ?? 0), BigInt(0))
      const uniqueWinners = entries.length
      return { totalWinnings, uniqueWinners }
    } catch { return { totalWinnings: BigInt(0), uniqueWinners: 0 } }
  })()

  const biggest = (() => {
    try {
      const list = (drawHistory as any[] | undefined) ?? []
      if (list.length === 0) return undefined
      let max = BigInt(0)
      let item: any = undefined
      for (const d of list) {
        const p = BigInt(d.prizeAmount ?? 0)
        if (p > max) { max = p; item = d }
      }
      return item ? { amount: max, id: Number(item.id), timestamp: Number(item.timestamp) } : undefined
      ...
      <main className="p-4 md:p-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          <Card className="bg-white/5 border-purple-500/20 backdrop-blur-sm">
            ...
            <CardContent>
              <div className="text-2xl font-bold">{`${Number(formatUnits((totals.totalWinnings ?? BigInt(0)) as bigint, 6)).toLocaleString()} USDC`}</div>
              <div className="flex items-center text-sm text-green-400">
                <TrendingUp className="h-4 w-4 mr-1" />
                Across {typeof currentDrawId !== 'undefined' ? Number(currentDrawId as bigint) : 0} draws
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white/5 border-purple-500/20 backdrop-blur-sm">
            ...
            <CardContent>
              <div className="text-2xl font-bold">{totals.uniqueWinners}</div>
              <div className="text-sm text-gray-400">All-time unique addresses</div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white/5 border-purple-500/20 backdrop-blur-sm">
            ...
            <CardContent>
              <div className="text-2xl font-bold">{biggest ? `${Number(formatUnits(biggest.amount as bigint, 6)).toLocaleString()} USDC` : '—'}</div>
              <div className="text-sm text-gray-400">{biggest ? `Draw #${biggest.id}` : '—'}</div>
            </CardContent>
          </Card>
        </div>
